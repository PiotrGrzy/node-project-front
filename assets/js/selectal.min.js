var Selectal = function(e) {
	...
	(this.selectGroup = document.createElement('div')),
		...
		(this.selectedItem.innerHTML = s[0].innerHTML),
	...
	for (var o = 0; o < s.length; o++) {
		...
		(n.className = 'selectal-dropdown-option'),
			(n.id = s[o].value),
			(n.innerHTML = s[o].innerHTML),
			this.dropdown.appendChild(n);
	}
	this.selectGroup.appendChild(this.dropdown);
	t.id;
	(this.input = document.createElement('input')),
		(this.input.type = 'hidden'),
		(this.input.value = s[0].value),
		(this.input.id = t.id),
		this.selectGroup.appendChild(this.input),
		t.parentNode.insertBefore(this.selectGroup, t.nextSibling),
		t.remove(),
		(this.options = addEventListeners(this));
};
function addEventListeners(o) {
	var e = document.querySelectorAll('.selectal-dropdown-option');
	o.selectBtn.addEventListener('click', function() {
		this.parentNode.querySelector('.selectal-dropdown');
		o.toggleDropdown();
	});
	for (var t = 0; t < e.length; t++)
		e[t].addEventListener('click', function() {
			var e = this.parentNode.parentNode,
				...
				s = (e.querySelector('.selectal-btn'), this.innerHTML);
...
function toggleDropdown() {
	this.dropdown.classList.contains('selectal-hidden')
...
function removeEventListener(e, t) {
	this.input.removeEventListener(e, t);
}
function getValue() {
